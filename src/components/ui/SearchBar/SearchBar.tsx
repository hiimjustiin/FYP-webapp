...
const SearchBar: React.FC<SearchBarProps> = ({
    ...
    const shouldShowResults = showResults && isFocused && (query.trim() !== '' || showAll);

    return (
        <div ref={containerRef} className={`searchbar-container ${className}`}>
            {/* Search Input */}
            <div className={`searchbar-input-wrapper ${shouldShowResults ? 'searchbar-input-wrapper--active' : ''}`}>
            ...
            {shouldShowResults && (
                <div className="searchbar-results">
                    <div className="searchbar-results-header overline">
                        {showAll ? `All Members (${filteredResults.length})` : 'Results'}
                    </div>
                    ...
                    <ul className="searchbar-results-list">
                        {filteredResults.map(member => (
                            ...
                            >
                                ...
                                />
                                <span className="searchbar-result-name subtitle-3">{member.name}</span>
                            </li>
                        ))}
                    </ul>
