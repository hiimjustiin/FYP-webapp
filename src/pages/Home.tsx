...
const ALL_PROJECTS: Project[] = [
  {
    ...
    summary:
      "Measured heat stress across five districts; proposed greening strategies.",
    submissions: [
      {
        id: "p1-s1",
        name: "Draft 1",
...
const Home = () => {
  ...
  const filteredProjects = useMemo<Project[]>(() => {
    ...
    return ALL_PROJECTS.filter((p) => p.date >= s && p.date <= e).sort(
      (a, b) => b.date.getTime() - a.date.getTime()
    ); // most recent first
  }, [startDate, endDate]);

  // Project dropdown options
  const projectOptions: DropdownOption[] = filteredProjects.map((p) => ({
    id: p.id,
    label: p.name,
  }));
  ...
  const submissionOptions: DropdownOption[] = (
  ...
  const selectedSubmission = selectedSubmissionId
    ? (selectedProject?.submissions ?? []).find(
  ...
  const lineChartData = useMemo(() => {
    ...
    return subs.map((s) => {
      const row: { submission: string; [key: string]: number | string } = {
        submission: s.name,
      };
    ...
    () =>
      selectedDimIds
        .map((id) => {
          ...
          return d
            ? { id: d.id, text: d.label, color: VARIANT_COLORS[d.variant] }
            : null;
        })
              ...
              <div className="flex items-center flex-wrap gap-4">
                ...
                >
                  <Dropdown
                    ...
                    className="w-full min-w-0"
                    customTriggerText={
                      selectedSubmission ? selectedSubmission.name : undefined
                    }
                  />
                </div>
                            ...
                            <tbody>
                              {DIMENSIONS.map(({ id, label }) => {
                                ...
                                return (
                                  ...
                                  >
                                    <td className="px-3 py-1 rounded-l-xl body-2">
                                      {label}
                                    </td>
                                    <td className="px-3 py-1 body-2 font-medium">
                                      {s.personal}
                                    </td>
                                    <td className="px-3 py-1 rounded-r-xl body-2">
                                      {s.classAvg}
                                    </td>
                                  </tr>
                                );
                    ...
                    <div className="rounded-2xl border border-[var(--color-grey-25)] bg-[var(--color-grey-05)]/40 p-3">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center gap-3">
                          <Button variant="green" onClick={selectAllDims}>
                            Select All
                          </Button>
                          <Button variant="grey" onClick={clearAllDims}>
                            Clear All
                          </Button>
                        </div>
                      </div>
                      ...
                      <div className="mt-2">
                        <p className="caption">
                          {selectedDimIds.length} of {DIMENSIONS.length}{" "}
                          dimensions selected
                        </p>
                      </div>
                    </div>
                    ...
                    <div>
                      {chartDimsForLine.length === 0 ? (
                        <div className="h-[420px] flex flex-col items-center justify-center text-[var(--color-grey-55)] gap-3">
                          <p className="text-[var(--color-grey-55)] subtitle-2">
                            No dimensions selected.
                          </p>
                          <p className="text-[var(--color-grey-55)] caption">
                            Select at least one dimension to display the line
                            chart.
                          </p>
                        </div>
