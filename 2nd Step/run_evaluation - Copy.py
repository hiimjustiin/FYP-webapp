...
def main():
    ...
    for index in range(start_idx, end_idx):
        ...
        for dim in DIMENSIONS:
            ...
            # Store clean quantitative alignment counts in columns
            row_metrics[f'{dim}_Aligned_Count'] = len(alignment.get("aligned", []))
            row_metrics[f'{dim}_Human_Only_Count'] = len(alignment.get("human_only", []))
            row_metrics[f'{dim}_AI_Only_Count'] = len(alignment.get("ai_only", []))
            
            # Store the raw text breakdowns directly in adjacent columns for manual qualitative validation
            row_metrics[f'{dim}_Shared_Points'] = ", ".join(alignment.get("aligned", []))
            row_metrics[f'{dim}_Points_AI_Missed'] = ", ".join(alignment.get("human_only", []))
            row_metrics[f'{dim}_Points_Human_Missed'] = ", ".join(alignment.get("ai_only", []))
